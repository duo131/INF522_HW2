...
def RunPredict(k,datasize):
    ...
    listB = []
    line = 0
    datafile = 'LA_CRIMES10to14_F2.csv' #get data
    ...
    for i in range(1, 6):
            if i != k:
                dataString = 'SP' + str(i) + '.csv'
                data_train = RelistData(dataString)
                random_list = Randomlist(datasize,len(data_train))
                randomrow = 0
                # for line in range(len(data_train)):
                for line in range(datasize):
                    randomrow = random_list[line]
                    listA.append([data_train[randomrow][2]  , data_train[randomrow][3], data_train[randomrow][4], data_train[randomrow][5] , data_train[randomrow][8], data_train[randomrow][11] ]) # data_train[randomrow][9] , data_train[randomrow][10],
                    listB.append(data_train[randomrow][6])
                    line += 1
    """
    #single file
    data_train = data_input
    for line in range(len(data_train)):
        listA.append([data_train[line][2]  , data_train[line][3], data_train[line][4], data_train[line][5] , data_train[line][8], data_train[line][9] , data_train[line][10] , data_train[line][11] ])
